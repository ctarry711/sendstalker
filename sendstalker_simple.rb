...
def scrape_sendage_sends(users, num_sends)
  ...
  users.each do |user|
    ...
    sendlist[0..num_sends].each do |climb|
      ...
      send = {name: name, location: location, climb_type: climb_type, grade: grade, send_type: send_type, date: date, rating: rating, comment: comment}
      all_sends[username] << send
    end
    puts "#{user} sends have been fetched: "
  end

  puts all_sends
end

users = ["christarry","zach-watson", "kierstenepps", "sam-4529"]
scrape_sendage_sends(users, 5)
