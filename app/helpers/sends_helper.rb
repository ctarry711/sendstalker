module SendsHelper
  ...
  def scrape_sendage_sends(followee, num_sends)
    ...
    realname_loc = doc.css('.profile-info .col .top')[0]

    if realname_loc
      realname = realname_loc.content
      profile_pic_path = doc.css('.profilepic img')[0]['src']
      user_info = {id: user_id, realname: realname, username: username, profile_pic: profile_pic_path}
      user_sends = []
      ...
      sendlist[..num_sends-1].each do |climb|
        ...
        # area_info = scrape_area_info(area_id)
        # area_name = area_info["name"]
        # area_parents = area_info["parents"].map {|i| i["name"]}
        # area_parents = []
        # area_name = "area name"
        
        user_sends << {realname: realname, name: name, profile_pic: profile_pic_path, area_id: area_id, climb_type: climb_type, grade: grade, send_type: send_type, date: date, rating: rating, comment: comment, slug: slug}
      end
    end
    return [user_info, user_sends]
  end
